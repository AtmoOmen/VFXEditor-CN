    ...
    public class ActionTabPap : SelectTab<ActionRowPap, Dictionary<string, Dictionary<string, string>>>
    {
        ...
        public override void LoadSelection( ActionRowPap item, out Dictionary<string, Dictionary<string, string>> loaded )
        {
            loaded = new Dictionary<string, Dictionary<string, string>> {
                { "开始", SelectDataUtils.FileExistsFilter( SelectDataUtils.GetAllSkeletonPaths( item.StartPath ) ) },
                { "结束", SelectDataUtils.FileExistsFilter( SelectDataUtils.GetAllSkeletonPaths( item.EndPath ) ) },
                { "受击", SelectDataUtils.FileExistsFilter( SelectDataUtils.GetAllSkeletonPaths( item.HitPath ) ) }
            };
        }
